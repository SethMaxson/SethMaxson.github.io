...
(function (Characters) {
    ...
    Characters.EquipmentModel = EquipmentModel;
    ...
    Characters.PersonPhysicalFeature = PersonPhysicalFeature;
    class Person {
    ...
    Characters.Person3DEquipment = Person3DEquipment;
    ...
    Characters.MiniatureBase = MiniatureBase;
    const equipment = [
    ...
    Characters.getRandom = getRandom;
                ...
                if (targetMesh != undefined) {
                    ...
                    if (stylo.texture != undefined) {
                        ...
                        if (typeof stylo.color === "string" && stylo.color.startsWith("#")) {
                            ...
                            hat.material.color.set(FOUR.Color(stylo.color));
    ...
    function getHumanoid(x, y, z, personInfo, gear) {
        if (personInfo != undefined) {
            ...
            person.getObjectByName("Torso").material = material;
            person.getObjectByName("Torso").receiveShadow = true;
            person.getObjectByName("EyeLids").material = material;
    ...
    Characters.getHumanoid = getHumanoid;
    function PersonLoader(callback, hideBase = false) {
        ...
        manager.onLoad = function () {
            ...
            let torsoMesh = person.getObjectByName("Torso");
            torsoMesh.castShadow = true;
            torsoMesh.receiveShadow = true;
            torsoMesh.receiveShadow = false;
            torsoMesh.frustumCulled = false;
            let eyelids = person.getObjectByName("EyeLids");
        ...
        loader.load('/res/models/badHair.glb', function (gltf) {
            gltf.scene.traverse(function (node) {
                hat.add(node.clone());
                if (node.material)
                    Characters.hairOptions.push(node.name);
            });
        }, undefined, function (error) {
            console.error(error);
        });
    ...
    Characters.PersonLoader = PersonLoader;
    ...
    Characters.updateHumanoid = updateHumanoid;
    ...
    Characters.removeFromBone = removeFromBone;
    ...
    Characters.getSprite = getSprite;
