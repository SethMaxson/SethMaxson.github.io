...
function getRaceOptionEntry(item) {
    ...
    if (item.hasOwnProperty('subraces')) {
        ...
        for (let index = 0; index < item.subraces.length; index++) {
            ...
            subraces.append($(`<li class="subrace-option" name="${subItem.name}">
					<div class="header">
...
function showMenu() {
    ...
    const a = newpage.element.find(".save-character")[0];
...
function showLoadPlayerSelectMenu() {
    $.ajax({ crossDomain: true, url: "/sf/res/data/characters/index.json", dataType: 'json' }).done(function (returnedData) {
        var players = "";
...
class ModalPopup {
    constructor(id, back, header, next) {
        this.id = id || "";
        back = back || "&lt; Prev";
        header = header || "";
        next = next || "Next >";
...
/************ Modal Pages ************/
class CreationPage {
    constructor(parent, id, back, header, next) {
        this.parent = parent;
        this.id = id || "";
        back = back || "&lt; Prev";
        header = header || "";
...
class DetailsPage extends CreationPage {
    constructor(parent) {
        ...
        autocomplete(this.$("#homeworld-input")[0], worlds);
    }
    update() {
    }
    show() {
...
class RacePage extends CreationPage {
    ...
    update(isChoose, name = "", subraceName) {
        ...
        getRaces().done(function (returnedData) {
            ...
            if (isChoose || name.length == 0) {
                ...
                subraceName = subraceName || "";
                var item = ccdm.races.filter(function (entry) {
                    return entry.name === name;
                })[0];
                if (subraceName.length > 0) {
                    item = item.subraces.filter(function (entry) {
                        return entry.name === subraceName;
                    })[0];
                }
                const id = item.hasOwnProperty('ID') ? item.ID.toLowerCase() : item.name.toLowerCase();
...
class ClassPage extends CreationPage {
    ...
    select(name) {
        ...
        var item = ccdm.classes.filter(function (entry) {
            return entry.name === name;
        })[0];
...
class AbilityPage extends CreationPage {
    constructor(parent, lvl) {
        ...
        this.$(".base").each((e, o) => {
            $(o).on("change", function (e) {
                ...
                isNaN(o) ? this.value = "10" : this.value = Math.max(Math.min(o, max), min).toString();
                changeTotal();
            });
        });
    ...
    finish() {
        const [s, d, c, i, w, ch] = this.$("input.base").get();
...
class ProgressionPage extends CreationPage {
    constructor(parent) {
        ...
        this.$(".level-select").on("change", function () {
            target.update(parseInt($(this).val()));
        });
        this.$("button.select").on("click", function () {
...
class CharacterLevelPage extends CreationPage {
    constructor(parent, lvl) {
        ...
        if (lvl % 2 != 1)
            feat.attr("disabled", "").attr("readonly", "");
        if (lvl % 5 != 0)
            asi.attr("disabled", "").attr("readonly", "");
        this.content = `
    ...
    update() {
        ...
        this.classLevel = this.parent.character.classLevel(this.level.class.name, this.characterLevel);
        // new, not functional
        const spentSkillRanks = this.level.skillRanks.ranks.reduce((a, b) => a + b, 0);
        // /new, not functional
        if (this.level.class.name !== undefined && this.level.class.name !== "") {
            ...
            getClasses().done(function (returnedData) {
                ...
                let clsData = returnedData.items.filter(function (entry) {
                ...
                if (spentSkillRanks == spendableSkillRanks)
                    target.$(".skill-ranks").addClass("complete");
                else
                    target.$(".skill-ranks").removeClass("complete");
            });
        }
...
class ClassLevelPage extends CreationPage {
    ...
    update(subclassOnly = false) {
        ...
        getClasses().done(function (returnedData) {
            ...
            var cls = returnedData.items.filter(function (entry) {
                return entry.name === name;
            })[0];
            target.classObj = cls;
            ...
            if (!subclassOnly) {
                ...
                for (let index = 0; index < features.length; index++) {
					...
					`);
                    if (e.hasOwnProperty('choice'))
                        opt.addClass("has-options");
    ...
    displayClassFeature(className, lvl, name) {
        if (this.classObj != undefined) {
            let target = this;
            var clvl = this.classObj.levels[lvl];
            var features = clvl.hasOwnProperty("features") ? clvl.features : [];
            var feat = features.filter(function (entry) {
                return entry.name === name;
            })[0];
    ...
    displaySubclassFeature(className, lvl, name) {
        if (this.classObj != undefined) {
            let target = this;
...
class SkillLevelPage extends CreationPage {
    constructor(parent) {
        ...
        getClasses().done(function (returnedData) {
            var items = returnedData.items;
            var clsData = items.filter(function (entry) {
                return entry.name === name;
            })[0];
            const sr = clsData != undefined ? parseInt(clsData.skillRanks) : 0;
            const spentRanks = lvlRanks.ranks.reduce((a, b) => a + b, 0);
            target.$(".modal-content").append($(`<div class="menu">
				<div class="label">
					Remaining Ranks:
    ...
    finish() {
        ...
        this.$("ul.options li .value input").each(function () {
            val += $(this).val() + "|";
            lvlRanks.ranks[i] = parseInt($(this).val()) - parseInt($(this).attr("min"));
            i++;
        });
...
class AbilityLevelPage extends CreationPage {
    ...
    checkbox(skill) {
        const rr = this.$(".remaining-ranks");
        const e = this.$(`ul.options li.${skill} input.checkbox`);
        if (e.prop("checked") == false) {
            this.$(`ul.options li.${skill} .after input`).val(this.$(`ul.options li.${skill} .before input`).val());
        }
        else {
            if (rr.val() > 0) {
                this.$(`ul.options li.${skill} .after input`).val(this.getUpgradedScore(parseInt(this.$(`ul.options li.${skill} .before input`).val())));
            }
            else {
...
function classLevelOptionsCount(data, className, level) {
    var result = 0;
    var cls = data.filter(function (entry) {
