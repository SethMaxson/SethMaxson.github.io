...
{
	spell: SpellManager|undefined = undefined;
}
...
function populatePage1(char: Character) {
...
function populatePage2(char: Character) {
	var feats = "";
	...
	getFeats().done(function (returnedData) {
		...
		for (let i = 0; i < f.length; i++) {
			var feat = items.filter(function (entry: Feat) {
...
function getClassDisplayString(char: Character){
...
function populateSpells(char: Character) {
	var spls = char.spellsKnown;
	var slots = char.spellSlots;
	var sortedSpells: string[][] = [
		[], [], [], [], [], [], []
	];
...
function createSkillRow(skill: string, ability: string, trainedOnly: boolean, armorPenalty: boolean) {
...
function createProfessionSkillRow(skill: string, ability: string, trainedOnly: boolean) {
	const abUC = ability.charAt(0).toUpperCase() + ability.slice(1);
	const toHTML = trainedOnly == true? "&dagger;": "";
	return $(`
...
function createAmmoCheckboxes(line: number, index: number) {
...
function modCapacity()
{
	$(".ammo-line").each(function() {
		var capacity = parseInt($(this).find(".capacity").val() as string) || 20;
...
function updateAC() {
	...
	const MAC = parseInt($("#KACTotal").val() as string) + 8 || 8;
	$("#MACTotal").val(MAC);
}
function sum(srcfields: string[]) {
	var total = 0;
	for(var i=0; i < srcfields.length; i++)
	{
		let field = document.getElementById(srcfields[i]) as HTMLInputElement;
		if(field)
		{
...
function populateName(charname: string = "")
{
...
function updateProficiencies(char: Character) {
...
function ClearInputs(){
	...
	(document.getElementById("alwaystrue") as HTMLInputElement).checked=true;
	(document.getElementById("AmmoCapacity1") as HTMLInputElement).value="20";
	(document.getElementById("AmmoCapacity2") as HTMLInputElement).value="20";
	(document.getElementById("AmmoCapacity3") as HTMLInputElement).value="20";
	(document.getElementById("AmmoCapacity4") as HTMLInputElement).value="20";
	(document.getElementById("AmmoCapacity5") as HTMLInputElement).value="20";
	(document.getElementById("Faction1") as HTMLInputElement).value="Acquisitives";
	(document.getElementById("Faction2") as HTMLInputElement).value="Dataphiles";
	(document.getElementById("Faction3") as HTMLInputElement).value="Exo-guardians";
	(document.getElementById("Faction4") as HTMLInputElement).value="Second Seekers";
	(document.getElementById("Faction5") as HTMLInputElement).value="Wayfinders";
	(document.getElementById("Faction6") as HTMLInputElement).value="Other";
	(document.getElementById("Faction7") as HTMLInputElement).value="Other";

	(document.getElementById("BackPack") as HTMLInputElement).value="0";
...
function CalcAllSkills() {
	...
	$("#Skills tr.skill").each(function(){
		...
		var sr = parseInt($(this).find(".ranks").val() as string) || 0;
		spentRanks += sr;
	});

	$("#TotalSkillSpent").val(spentRanks);
}
function SumUpSkill(skill: HTMLElement, trainedonly: boolean, applyACP: boolean)
{
	const inputs = $(skill).find("input") as JQuery<HTMLInputElement>;
	...
	var ACP = parseInt($("#ACP").val() as string) || 0;
...
{
	var sStrMod=(document.getElementById('StrUpScore') as HTMLInputElement).value;
	if(sStrMod=="")
	{
		sStrMod=(document.getElementById('StrScore') as HTMLInputElement).value;
	}
	var eStrMod=parseInt(sStrMod)
	if(!isNaN(eStrMod))
	{
		eStrMod+=parseInt($("#BackPack").val() as string);
		if((document.getElementById('Mercenary6') as HTMLInputElement).checked)
		{
			eStrMod+=1
		}
		var uBulk = Math.trunc(eStrMod/2);
		(document.getElementById('CapacityU') as HTMLInputElement).value = "" + uBulk;
		(document.getElementById('CapacityE') as HTMLInputElement).value = (uBulk+1) + "-" + eStrMod;
		(document.getElementById('CapacityO') as HTMLInputElement).value = (eStrMod + 1).toString();
	}
}
function CalculateBulk()
{
	...
	for (var i=1; i <= 40; i++)
	{
		let box = document.getElementById("ItemBulk"+i) as HTMLInputElement;
		if(box.value=="L")
			totalbulk += 1;
	...
	(document.getElementById("CapacityC") as HTMLInputElement).value=Math.trunc(totalbulk/10).toString();
}
function SumUp(dstfield: string, srcfields: string[])
{
	var total = 0;
	for(var i=0; i < srcfields.length; i++)
	{
	...
	(document.getElementById(dstfield) as HTMLInputElement).value = total.toString();
}
function displaySpell(spellName: string)
{
	if (csdm != undefined) {
		if (csdm.spell != undefined) {
			let spell = csdm.spell.get(spellName);
