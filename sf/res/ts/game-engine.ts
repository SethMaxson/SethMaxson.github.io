...
declare function speak(speaker: string | Dialog.Dialog[], speech: number | string, callback ?: Function, character ?: Engine.Entity): void;

export var main: Engine.Main;
...
export namespace Engine
{
	...
	export class PersonMotion extends Motion
	{
		...
		update(main: Main, delta: number)
		{
			if (this.canMove)
			{
				...
				if (this.collisions.front.length > 0) this.velocity.z = Math.max(0, this.velocity.z);
				if (this.collisions.rear.length > 0) this.velocity.z = Math.min(0, this.velocity.z);
	...
	*/
	class PlayerMotionDeprecated extends Motion
	{
		...
		update(main: Main, delta: number, controls?: PlayerControls, firstPerson: boolean = false)
		{
			...
			if (this.canMove && controls != undefined)
			{
				...
				if (moveForward || moveBackward) this.velocity.z -= newDirection.z * this.baseSpeed * delta * accelerationModifier;
				if (moveLeft || moveRight) this.velocity.x -= newDirection.x * this.baseSpeed * delta * accelerationModifier;
	...
	function FloorCollision(motion: PersonMotion|PlayerMotion, delta: number, jumped: boolean = false, name: string = "Player")
	{
	...
	function ApplyGravity(motion: PlayerMotion, delta: number, mass: number = 100)
	{
	...
	export class PlayerMotion extends Motion
	{
		...
		update( main: Main, delta: number, controls?: PlayerControls, firstPerson:boolean = false ) {
			...
			if (this.canMove && controls != undefined)
			{
				...
				// Calculate acceleration and new velocity
				var accelerationModifier = (this.sprinting == true) ? 2 : 1;
				if ( moveForward || moveBackward || moveLeft || moveRight ) this.velocity.z -= this.baseSpeed * delta * accelerationModifier;
				this.velocity.z = Math.round(this.velocity.z * 1000) / 1000;
				this.velocity.x = 0;
		...
		jump()
		{
			if (this.airborne) this.jumpCounter = Math.max(this.jumpCounter, 1);
			...
			if (this.motion.canMove === true) {
				if (!this.firstPerson) {
					if (this.motion.horizontalSpeed > 0.2) {
						...
						(this.mesh.userData.mixer as THREE.AnimationMixer).timeScale = 2 * this.motion.horizontalSpeed;
					}
					else
					{
						...
						(this.mesh.userData.mixer as THREE.AnimationMixer).timeScale = 2;
					}
				}
			}
		}
