...
export class HUD
{
	...
	showTooltip(text: string, duration: number = 2000)
	{
	...
	{
		return (this.html.find(".spells") as JQuery<HTMLDivElement>).is(":visible");
	}
	set showSpellMenu(value: boolean)
	{
		let spellMenu = (this.html.find(".spells") as JQuery<HTMLDivElement>);
		if (value == true) {
			spellMenu.show();
		} else {
			spellMenu.hide();
