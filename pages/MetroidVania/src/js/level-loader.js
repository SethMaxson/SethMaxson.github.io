...
export const parseLevel = (levDat) => {
    $.ajax({
        dataType: "text",
    ...
    if (levelDefinitions.length > 1) {
        ...
        for (let i = 0; i < specialDataLines.length; i++) {
            ...
            if (line[0] == "#Door" || line[0] == "#door") {
                ...
                }, line[3], isNaN(parseInt(line[4])) ? 0 : parseInt(line[4]), isNaN(parseInt(line[5])) ? 0 : parseInt(line[5]), isNaN(parseInt(line[4])), isNaN(parseInt(line[5])), line[6], line[7] == "t"));
            }
    ...
    for (let i = 0; i < level.length; i++) {
        let bigBreak = false;
        let pos = { x: stride * tileSize, y: depth * tileSize };
        switch (level[i]) {
            case "\n": // new line
                stride = 0;
                depth++;
...
//# sourceMappingURL=level-loader.js.map