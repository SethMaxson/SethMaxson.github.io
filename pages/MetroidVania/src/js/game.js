...
import { levels, parseLevel } from "./level-loader.js";
import { bindKeyHandlers, userController } from "./controller.js";
import { globals, boxCollision } from "./util.js";
...
class Game {
    ...
    tick() {
        ...
        if (this.gameState == GAME_STATES.GAME_PLAYING && !this.camera.faded) {
            ...
            for (let i = 0; i < enemies.length; i++) {
                if (enemies[i] !== null) {
                    if (enemies[i].isEnemy) { // when player is attacking, see if they hit an enemy
                        if (this.player.attackVolume !== null) {
                            let result = boxCollision(this.player.attackVolume, enemies[i]);
                            if (result)
                                enemies[i].takeDamage(this.player.attackPower);
                        }
