...
{
	actionResets: { attack: boolean; attackPressed: boolean; animatingAttack: boolean; onGround: boolean; interactPressed: boolean; jumpPressed: boolean; canAirJump: boolean; };
	...
	attackPower: number;
	boundaries: { topLeft: IPoint2D; topRight: IPoint2D; rightTop: IPoint2D; right: IPoint2D; rightBottom: IPoint2D; bottomLeft: IPoint2D; bottomRight: IPoint2D; leftTop: IPoint2D; left: IPoint2D; leftBottom: IPoint2D; };
	boundaryCollision: { top: boolean; right: boolean; bottom: boolean; left: boolean; };
	...
	constructor(args: { spawnPos: IPoint2D })
	{
		args = args || {};
		let startVals = {
			size: { w: 58, h: 107 },
	...
	animate()
	{
		if (!this.actionResets.animatingAttack)
		{
			...
			} else if ((userController.right) && (this.vel.y === 0))
			{ //running right on the ground
				this.playerState = PLAYER_STATES.RUNNING_RIGHT;
			} else if ((userController.left) && (this.vel.y === 0))
			{ //running left on the ground
				this.playerState = PLAYER_STATES.RUNNING_LEFT;
			...
			{ //running right -> idle
				this.playerState = PLAYER_STATES.IDLE_RIGHT;
			} else if ((this.playerState === PLAYER_STATES.RUNNING_LEFT) && (this.vel.y > 0))
			{ //running left -> falling
				this.playerState = PLAYER_STATES.FALLING_LEFT;
			} else if ((this.playerState === PLAYER_STATES.RUNNING_LEFT) && (this.vel.y < 0))
			{ //running left -> jumping
				this.playerState = PLAYER_STATES.JUMPING_LEFT;
			} else if ((this.playerState === PLAYER_STATES.RUNNING_RIGHT) && (this.vel.y > 0))
			{ //running right -> falling
				this.playerState = PLAYER_STATES.FALLING_RIGHT;
			} else if ((this.playerState === PLAYER_STATES.RUNNING_RIGHT) && (this.vel.y < 0))
			{ //running right -> jumping
				this.playerState = PLAYER_STATES.JUMPING_RIGHT;
			} else if ((this.playerState === PLAYER_STATES.JUMPING_LEFT) && (this.vel.y > 0))
			{ //jumping left -> falling left
				this.playerState = PLAYER_STATES.FALLING_LEFT;
			} else if ((this.playerState === PLAYER_STATES.JUMPING_RIGHT) && (this.vel.y > 0))
			{ //jumping right -> falling right
				this.playerState = PLAYER_STATES.FALLING_RIGHT;
			...
			{ //idle right -> jumping
				this.playerState = PLAYER_STATES.JUMPING_RIGHT;
			} else if ((this.playerState === PLAYER_STATES.FALLING_LEFT) && this.vel.y === 0)
			{ //falling left -> on the ground
				this.playerState = PLAYER_STATES.IDLE_LEFT;
			} else if ((this.playerState === PLAYER_STATES.FALLING_RIGHT) && this.vel.y === 0)
			{ //falling right -> on the ground
				this.playerState = PLAYER_STATES.IDLE_RIGHT;
			} else if ((this.playerState === PLAYER_STATES.JUMPING_LEFT) && userController.right)
			{ //jumping left -> turning to the right
				this.playerState = PLAYER_STATES.JUMPING_RIGHT;
			} else if ((this.playerState === PLAYER_STATES.JUMPING_RIGHT) && userController.left)
			{ //jumping right -> turning to the left
				this.playerState = PLAYER_STATES.JUMPING_LEFT;
			} else if ((this.playerState === PLAYER_STATES.FALLING_LEFT) && userController.right)
			{ //falling left -> turning to the right
				this.playerState = PLAYER_STATES.FALLING_RIGHT;
			} else if ((this.playerState === PLAYER_STATES.FALLING_RIGHT) && userController.left)
			{ //falling right -> turning to the left
				this.playerState = PLAYER_STATES.FALLING_LEFT;
			} else if ((this.playerState === PLAYER_STATES.IDLE_LEFT) && this.vel.y > 0)
			{ //idle left -> falling left
				this.playerState = PLAYER_STATES.FALLING_LEFT;
			} else if ((this.playerState === PLAYER_STATES.IDLE_RIGHT) && this.vel.y > 0)
			{ //idle right -> falling right
				this.playerState = PLAYER_STATES.FALLING_RIGHT;
			} else if (this.playerState === PLAYER_STATES.ATTACKING_LEFT)
			{ //attacking left -> idle
				this.playerState = PLAYER_STATES.IDLE_LEFT;
			} else if (this.playerState === PLAYER_STATES.ATTACKING_RIGHT)
			{ //attacking right -> idle
				this.playerState = PLAYER_STATES.IDLE_RIGHT;
			}
		}
	}
	...
	attackAnimation()
	{
		if (this.sprites)
		{
			...
			{
				clearInterval(this.attackId as NodeJS.Timeout);
			} else
			{
				this.spriteIdx++;
			}
