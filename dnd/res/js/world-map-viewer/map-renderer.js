...
function getCitiesByContinent(destinationElement, continentName, specialTreatmentForHyperLinks = false) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        var cityCount = 0;
        $.when(getMapImageData(continentName), getCityData(), getMapLocationData(continentName)).done(function (cityMapImageDataRaw, continentSections, locationsRaw) {
            ...
            for (let i = 0; i < locations.length; i++) {
                ...
                destinationElement.append($(`<a href="${url}" class="point-of-interest smith village" style="left: calc(${locations[i].left} - 8px); top: calc(${locations[i].top} - 8px); ${specialTreatmentForHyperLinks && url != "#" ? " color:#ffff00;" : ""}" data-city="${locations[i].name}" data-continent="${continentName}" data-structure="map-location">
 ...
 */
function getCityMarkup(city, continent, cityMapImageData, specialTreatmentForHyperLinks = false) {
    let description = "";
 ...
 */
function attachPlacementHelper(mapElement) {
    ...
    $(document).ready(function () {
        ...
        $(document).on("click", "a.point-of-interest", function (e) {
            ...
            $.when(getCityObject($(this).attr("data-city"), $(this).attr("data-continent"), $(this).attr("data-structure") == "cities.json")).done(function (city) {
                //@ts-ignore
                window.modalWindow.displayCity(city);
            });
        });
    });
});
