"use strict";
var NPCGenFilterCategory;
(function (NPCGenFilterCategory) {
    NPCGenFilterCategory["Age"] = "age";
    NPCGenFilterCategory["Alignment"] = "alignment";
    NPCGenFilterCategory["Gender"] = "gender";
    NPCGenFilterCategory["Intelligence"] = "intelligence";
...
class NPCDeepGenerator {
     ...
     */
    getNPCAge(npc, rt, ageCategory) {
        /**Declare variable and set placeholder value. */
        let selectedAgeCategory = AgeCategory.Old;
        if (ageCategory == undefined) {
            selectedAgeCategory = weightedRandom(this.AgeList, this.totaledWeights.Age);
        }
            ...
            this.AgeList;
            selectedAgeCategory = weightedRandom(filteredAges, filteredWeight);
        }
    ...
    getNPCRacialAppearance(race) {
        if (RacialAppearance.hasOwnProperty(race)) {
            return RacialAppearance[race];
        }
        else {
     ...
     */
    getPronoun(gender, wordForms = ['they', 'she', 'he']) {
    ...
    randomizeNPC(npc, race, gender, age, alignment) {
 ...
 */
function filteredWeightedRandom(npc, filterable) {
 ...
 * @param filterable The filterable weighted key list to use
 */
function getFilteredWeightedKeyList(npc, filterable) {
    let applicable = {};
 ...
 * @param kle
 */
function isWeightedKeyListEntryApplicable(npc, kle) {
 ...
 */
function getRandomRaceFromList(availableRaces) {
    let totalWeight = 0;
    let weightedRaces = {};
    for (let i = 0; i < races.length; i++) {
        const e = races[i];
