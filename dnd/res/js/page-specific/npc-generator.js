...
$(document).ready(function () {
    ...
    $("#generate-npcs").on("click", function () {
        var race = undefined, gender = undefined, age = undefined, availableAlignments = undefined;
        var restrictRacesByAlignment = $("#alignment-restrict-races").prop("checked");
        // get species setting
        if ($("#race-select").val().length == 1) {
            race = $("#race-select").val()[0];
        }
        else if ($("#race-select").val().length > 1) {
            race = $("#race-select").val();
        }
        // get gender setting
        if ($("#gender-select").val().length == 1)
            gender = $("#gender-select").val()[0];
        // get age setting
        age = getFilterValue("#age-select");
        // get alignment setting
        if ($("#alignment-select").val() !== "null") {
            let alnVal = $("#alignment-select").val()?.toString();
            availableAlignments = ((alnVal.indexOf(",") != -1) ? alnVal.split(",") : [alnVal]);
            if (restrictRacesByAlignment) {
                race = getRaceByAlignment(availableAlignments);
            }
        }
        // create NPCs
        generateNPCs(race, gender, age, availableAlignments, $("#records-to-generate").val());
    });
    $("#loaded-sort-select").on("change", function () {
        sortNPCs(false, $(this).val());
    });
    $("#random-sort-select").on("change", function () {
        sortNPCs(true, $(this).val());
    });
...
function deleteNPC(index, isRando) {
...
function sortNPCs(isRando, property) {
    let npcCollection = isRando ? randomNPCs : loadedNPCs;
    npcCollection.sort(property);
    isRando ? renderRandomNPCs() : renderLoadedNPCs();
}
function transferNPCBetweenManagers(moveToRando, index) {
...
function removeNPCFromManager(manager, index) {
...
function generateNPCs(race, gender, age, alignment = [], number) {
...
function getFilterValue(jquerySelector) {
    let result = undefined;
    if ($(jquerySelector).attr("multiple")) {
        let baseVal = $(jquerySelector).val();
        if (baseVal.length > 1) {
            result = [];
