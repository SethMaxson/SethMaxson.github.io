...
$(document).ready(function(){
	...
	$("#map-container").mousedown(function() {
		...
		var zoomString = "scale(" + zoom + ")";
		var originLeft = $("#map-body").scrollLeft() as number + ($("#map-body").width() as number / 2);
		var originTop = $("#map-body").scrollTop() as number + ($("#map-body").height() as number / 2);
	...
	$('#map-body').on('wheel mousewheel', function(e){
		var delta;
		var startZoom = $("#map-zoom").val() as number * 1;
