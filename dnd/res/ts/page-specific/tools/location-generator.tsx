...
interface ILocationGeneratorNavProps
{
	SelectGenerator: { (value: string): void };
	SelectedGenerator: string;
}
class LocationGeneratorNav extends React.Component<ILocationGeneratorNavProps> {
	render()
	{
		return (
			<nav className="navbar navbar-light bg-light bg-gradient border-top border-bottom">
				<div className="container-fluid">
					<ul className="navbar-nav me-auto mb-2 mb-lg-0">
						<li className="nav-item">
							<select
								className="form-select"
								onChange={e => this.props.SelectGenerator(e.target.value)}
								value={this.props.SelectedGenerator}
							>
								{LocationGeneratorOptions.map((option) =>
									<option
										value={option.value}
										key={option.value}
									>
...
interface ILocationGeneratorProps
{
	tableData: ILocationGeneratorTables;
}
interface ILocationGeneratorState
{
	selectedGenerator: string;
}
class LocationGenerator extends React.Component<ILocationGeneratorProps, ILocationGeneratorState> {
	constructor(props: ILocationGeneratorProps)
	{
		super(props);
	...
	render()
	{
		return (
			<>
				<LocationGeneratorNav
					SelectedGenerator={this.state.selectedGenerator}
					SelectGenerator={this.updateSelectedGenerator}
				/>
	...
	updateSelectedGenerator = (value: string) =>
	{
		this.setState({ selectedGenerator: value });
	}
}

interface IIslandGeneratorProps
{
	tableData: ILocationGeneratorTables;
}
interface IIslandGeneratorState
{
	selectedGenerator: string;
}
class IslandGenerator extends React.Component<IIslandGeneratorProps, IIslandGeneratorState> {
	constructor(props: IIslandGeneratorProps)
	{
		super(props);
							...
							<div className="accordion-body p-0">
								<div className="accordion accordion-flush" id="standardSubracesAccordion">
									{this.props.tableData.subraces.standard.map((slimData, index: number) =>
										<RollableTableAccordionContainer
											ParentID="standardSubracesAccordion"
											Data={slimData}
											key={index}
										/>
									)}
							...
							<div className="accordion-body p-0">
								<div className="accordion accordion-flush" id="exoticSubracesAccordion">
									{this.props.tableData.subraces.exotic.map((slimData, index: number) =>
										<RollableTableAccordionContainer
											ParentID="exoticSubracesAccordion"
											Data={slimData}
											key={index}
										/>
									)}
...
interface IRollableTableAccordionContainerProps
{
	Data: ISlimReincarnationTableData;
	ParentID: string;
	ResultColumnLabel: string;
}
class RollableTableAccordionContainer extends React.Component<IRollableTableAccordionContainerProps> {
		...
		return (
			<div className="accordion-item">
				...
				<div
					...
					<div className="accordion-body p-0">
						{this.props.Data.preface?.map((paragraph, index: number) =>
						...
						/>
						{this.props.Data.postface?.map((paragraph, index: number) =>
							<ParagraphFromRawHTML text={paragraph} key={index} />
						)}
					</div>
				</div>
...
const LocationGeneratorTables: ILocationGeneratorTables = {
	distortions: {
		elemental: [
			{
				title: "Air",
...
ReactDOM.render(
	<LocationGenerator tableData={LocationGeneratorTables} />,
