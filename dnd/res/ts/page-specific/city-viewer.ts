...
$(document).ready(function(){
	...
	if (targetCity) {
		...
		$.ajax({
			...
			dataType: 'json',
			success: function (returnedData: ICityBrochure[]) {
				let city = returnedData.filter(function (entry) {
					return entry.name.toLowerCase() === targetCity.toLowerCase();
				})[0];
				if (city) {
...
function getAppropriateImageDimensions(imageSrc: string)
{
	let img = new Image();
	img.src = imageSrc;
	img.onload = function ()
...
function scaleImageSize(width: number, height: number): ImageSize
{
