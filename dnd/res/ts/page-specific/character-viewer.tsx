type CharacterStatus = "Fine" | "Fallen" | "Missing" | "Unknown";
...
class CharacterPortrait extends React.Component<ICharacterPortraitProps> {
	render() {
		return (
			<div className="portrait-frame float-end">
...
class CharacterProperties extends React.Component<ICharacterPropertiesProps> {
	render() {
		return (
			<p className="properties fst-italic fs-6">
				{this.props.alignment + ", "}
				<span dangerouslySetInnerHTML={{ __html: this.props.race + ", " }} />
				{this.props.class}
...
class CharacterTitles extends React.Component<ICharacterTitlesProps> {
	render() {
		if (this.props.titles.length > 0) {
			return (
				<p className="titles">{"Titles: " + this.props.titles.join(", ")}</p>
			);
		}
		else {
			return null;
...
class CharacterView extends React.Component<ICharacterViewProps, ICharacterViewState> {
...
class CharacterViewer extends React.Component<ICharacterViewerProps, ICharacterViewerState> {
	constructor(props: ICharacterViewerProps)
	{
		...
		let matchingCharacters: ICharacterSlideObject[] = [];
		if (characterName) {
...
const CHARACTERS: ICharacterSlideObject[] = [
	{
...
ReactDOM.render(
	<CharacterViewer characters={CHARACTERS.sort((a, b) => a.name > b.name && 1 || -1)} />,
	document.getElementById("character-viewer-panel")
);