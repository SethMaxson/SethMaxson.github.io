...
class Campaign1OriginalNotes extends React.Component<ICampaign1OriginalNotesProps> {
	render()
	{
		return (
			...
			<div className="container px-0">
				...
				<div className="tab-content container" id="myTabContent">
					...
					<div className="tab-pane row" id="campaign-2" role="tabpanel" aria-labelledby="campaign-2-tab">
						...
						<div className="accordion" id={"accordion" + (++campaignId)}>
							<SessionEntry accordionID={"accordion" + campaignId} date="12/31/2021" title="A New Hope" sessionNumber={++sessionId}>
								<SessionPartyList>
									- New Hero #1
									- New Hero #2
									- New Hero #3
			...
			<div className="accordion-item">
				<h2 className="accordion-header" id={id + "-heading"}>
					<button className="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target={"#" + id} aria-expanded="false" aria-controls={id}>
						<span className="col-auto">{this.props.date}</span>
						<span className="col-auto px-3 px-lg-4">-</span>
						<span className="col">{this.props.title}</span>
					</button>
				</h2>
				<div id={id} className="accordion-collapse collapse" aria-labelledby={id + "-heading"} data-bs-parent={"#" + accordionID}>
					<div className="accordion-body" style={{backgroundColor: "#f1f1f1", color: "#212529"}}>
						{this.props.children}
					</div>
				</div>
			</div>
...
class SessionPartyList extends React.Component<ISessionPartyListProps> {
	...
	{
		return (
			<div className="card text-muted mb-3 p-3">
				<small>
					Party:
					<ul className="party-list">
...
class SessionBody extends React.Component<ISessionBodyProps> {
	render()
	{
		return (
			<div>
				{(this.props.children as string).split(/\r?\n\r?\n/).map(value =>
					(value.trim().length > 0) &&
					<ParagraphFromRawHTML text={value.trim()} />
				)}
			</div>
		);
	}
...
class SessionNotes extends React.Component<ISessionNotesProps> {
	render()
	{
		let headerSplit = (this.props.children as string).trim().split("Party:");
		...
		return (
			<SessionEntry accordionID={"accordion" + this.props.campaignId} date={date} title={title} sessionNumber={this.props.sessionNumber}>
				<SessionPartyList>
					{party}
				</SessionPartyList>
				<SessionBody>
...
class CampaignNotes extends React.Component<ICampaignNotesProps> {
	render()
	{
		return (
			<div className="accordion accordion-flush" id={"accordion" + (this.props.campaignId)}>
				{(this.props.children as string).split(/\r?\n\s*\r?\n\s*\r?\n/).map((value: string, index: number) =>
					(value.trim().length > 0) &&
					<SessionNotes campaignId={this.props.campaignId} sessionNumber={index} key={index}>
						{value.trim()}
					</SessionNotes>
				)}
			</div>
