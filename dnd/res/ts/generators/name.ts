...
function getNameFromGender(gender: string, genderNeutralNames: string[], feminineNames: string[], masculineNames: string[]): string
{
	// define non-gendered names
	let nameOptions = genderNeutralNames;

	// add gender specific names
	if (gender == "male")
	{
		nameOptions = nameOptions.concat(masculineNames);

	} else
	{
		nameOptions = nameOptions.concat(feminineNames);
	}

	return randomize(nameOptions);
}

function getNameFromBlock(gender: string, age: string, block: IGenderedNameData): string {
	if (block.convention == "gendered") {
		return getNameFromGender(gender, block.neutral, block.feminine, block.masculine);
	}
	else {
		return "";
	}
