declare var getRandomInt: { (min: number, max: number): number };
...
// Updated to D&D
var buildingFlavour = [
  "has extremely well-armed security guards",
  ...
  "is obscured by large illusury magic ads",
  ...
  "you must scan your alignment on entry",
...
function getPopulation(size: string) {
  var pop = 0;
  switch (size) {
    case "empty":
        pop = 0;
...
function printPanel(alignment,population,type,government,qualities,item,size: keyof typeof numOfShops) {
  var $outputArea = $(".output.area").first();
  var storeOutput = $( "div.output.area" ).html();
  $outputArea.empty();

  indexCounter += 1;
  let indexString = "index" + indexCounter.toString();
...
function generateSettlement() {
  ...
  if (["lone tavern","camp site","farm"].includes(randType)) {
    ...
    randPop = (Math.round(getRandomInt(20000000000, 2000000000000)/1000000000)*1000000000).toString();
    randPop = randPop.replace(/\B(?=(\d{3})+(?!\d))/g, ",") + popPercentages()
  } else {
