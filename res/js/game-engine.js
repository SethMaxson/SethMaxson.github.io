...
class Motion extends THREE.Object3D {
	...
	update( camera, controls, objects, delta, fps ) {
		...
		if (this.canMove) {
			...
			if ( moveForward || moveBackward ) this.velocity.z -= this.direction.z * this.baseSpeed * delta * accelerationModifier;
			if ( moveLeft || moveRight ) this.velocity.x -= this.direction.x * this.baseSpeed * delta * accelerationModifier;
