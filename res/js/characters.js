	...
	if (style != undefined && style !== null) {
		if (style == "Head_BlueJay") {
				...
				if (e != undefined) {
					...
					if (doo.material.map != null) {
						if (typeof tColor === "string" && tColor.startsWith("#")) {
							doo.material.color = new THREE.Color(tColor).convertSRGBToLinear();
						}
					}
					else {
						if (typeof tColor === "string" && tColor.startsWith("#")) {
							doo.material = FOUR.Material({
								color: new THREE.Color(tColor).convertSRGBToLinear(),
								emissive: 0x000000,
			...
			if (e != undefined) {
				...
				hat.material = new FOUR.Material({
					color: 0xffffff,
					emissive: 0x000000,
					side: THREE.FrontSide,
					specular: 0x666666,
			...
			if (typeof color === 'string' && !color.startsWith("0x")) {
				...
				hat.material = FOUR.Material({
					color: new THREE.Color(color).convertGammaToLinear( 2.2 ),
					emissive: 0x000000,
					side: THREE.DoubleSide,
					// specular: 0x666666,
					shininess: 30
